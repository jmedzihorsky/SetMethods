xy.plot <-
	...
	{	
		...
		if (necessity == TRUE) {   
			...
			lab <- sprintf("Consistency Necessary Condition: %.3f \n Coverage Necessary Condition: %.3f", con, cov)
			cons.c <- paste("Consistency Necessary Condition",
							cons, sep = ": ")
			cove.c <- paste("Coverage Necessary Condition",
							cove, sep = ": ")
			...
			cove <- format(cov, digits = 3)
			storage.mode(cove) <- "numeric"
			lab <- sprintf("Consistency Sufficient Condition: %.3f \n Coverage Sufficient Condition: %.3f", con, cov)
		}
